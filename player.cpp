...
//! [2]
    ...
    //(tabs->verticalScrollBar())->setMinimumWidth(15);
    //tabs->verticalScrollBar()->setHidden(true);
    //tabs->verticalScrollBar()->adjustSize();
    //tabs->setVerticalScrollMode(QTreeView::ScrollPerPixel);
    //(tabs->verticalScrollBar())->setMaximumWidth(25);
    for (int i = 0; i <= database.bodyparts.size() - 1; i++) {
        ...
        if (!database.bodyparts.at(i).bilateral) {
            ...
            for (int j = 0; j < database.bodyparts.at(i).data.size(); j++) {
                ...
                if ((character->text(0)).compare("Asymmetrical arm swing")==0 || (character->text(0)).compare("Small strides") == 0) {
    ...
    //tabs->setStyleSheet(QString("padding:5px;"));
    tabs->setFixedWidth(300);
    connect(tabs, SIGNAL(itemClicked(QTreeWidgetItem*, int)), this, SLOT(needStop()));
    connect(tabs, SIGNAL(itemExpanded(QTreeWidgetItem*)), this, SLOT(needStop()));
    connect(tabs, SIGNAL(itemChanged(QTreeWidgetItem*, int)), this, SLOT(recordRTime(QTreeWidgetItem*)));
...
void Player::beginSurvey() {
    ...
    if (playlist->currentIndex() == 0) {
        message = QString::fromStdString("Practice video is about to begin!\n\nTarget is wearing a blue shirt. ");
    } else {
        message = QString::fromStdString("Video " + std::to_string(playlist->currentIndex()) + " is about to begin! " + postMessage);
    }
...
void Player::endSurvey() {
    ...
    if (playlist->currentIndex() != 0) {
        ...
        database.addToSequencePlayed3(std::to_string(playlist->currentIndex()));
    }
...
//        tabs->setItemWidget(comment.at(i),0,line);
        ((QLineEdit *)(tabs->itemWidget(comment.at(i), 0)))->clear();
    }
}
...
//                orderClickedOverall++;
                ...
                for (int j = 0; j < vIndex.size(); j++) {
                    if (vIndex.at(j) == playlist->currentIndex()) {
                        i1 = j;
                        break;
                    }
                }
                int i2 = -1;
                for (int j = i1; j < vTime.size(); j++) {
                    if (vTime.at(j) == player->position()) {
                        i2 = j;
                        break;
                    }
                }
                int i3 = -1;
                for (int j = i2; j < check.size(); j++) {
                    if (check.at(j) == boxes.at(i)) {
                        i3 = j;
                    }
                }
                database.addToRealTime1(rTime.at(i3));
                database.addToIntTime(iTime.at(i3));
...
//                database.addOrderClicks(std::to_string(orderChecked));
                database.addToSequencePlayed(std::to_string(playlist->currentIndex()));
...
//                    }
//                }
//                database.addToRealTime2(rTime.at(i3));
                int cut = ((showTime->text()).toStdString()).find("   ");
                string substring = ((showTime->text()).toStdString()).substr(0, cut);
                database.addToRealTime1(substring);
...
void Player::mousePressEvent(QMouseEvent *event) {
    ...
    if (posRX > 0 && posRX < videoWidget->width() && posRY > 0 && posRY < videoWidget->height() && (playlist->currentIndex() + 1) >= 1) {
        ...
        if (playlist->currentIndex()!=0) {
            ...
            database.addToSequencePlayed2(std::to_string(playlist->currentIndex()));
        }
        emit playClicked();
    }
}
...
void Player::inputParNum() {
    if (!playlist->isEmpty()) {
        ...
        inputNum.setLabelText("Please enter your participant number: ");
        inputNum.exec();
...
void Player::recordRTime(QTreeWidgetItem* t) {
    int startIndex = -1;
    for (int i = 0; i < vIndex.size(); i ++) {
        if (vIndex.at(i) == playlist->currentIndex()) {
            startIndex = i;
            break;
        }
    }
    if (startIndex == -1) {
        vIndex.push_back(playlist->currentIndex());
        vTime.push_back(player->position());
        int cut = ((showTime->text()).toStdString()).find("   ");
        string substring = ((showTime->text()).toStdString()).substr(0, cut);
        rTime.push_back(substring);
        iTime.push_back(QDateTime::currentMSecsSinceEpoch());
        check.push_back(t);
    } else {
        int beginIndex = -1;
        for (int i = 0; i < vTime.size(); i++) {
            if (vTime.at(i) == player->position()) {
                beginIndex = i;
                break;
            }
        }
        if (beginIndex == -1) {
            ...
            int atIndex = -1;
            bool here = false;
            for (int i = beginIndex; i < vTime.size(); i++) {
                if (check.at(i) == t) {
                    here = true;
                    atIndex = i;
                }
            ...
            if (here) {
                vIndex.erase(vIndex.begin() + atIndex);
                check.erase(check.begin() + atIndex);
                vTime.erase(vTime.begin() + atIndex);
                rTime.erase(rTime.begin() + atIndex);
                iTime.erase(iTime.begin() + atIndex);
            } else {
                vIndex.push_back(playlist->currentIndex());
                vTime.push_back(player->position());
                int cut = ((showTime->text()).toStdString()).find("   ");
